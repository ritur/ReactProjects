...
const ArchiveEmailsListItem = ({ email, emailFrom, myid, myusername, emailsTo, onChange,value,selectedItems, checked,onSelectEmail, handleMarkAsStar, onReadEmail, getTaskLabelNames }) => (
    <li className="d-flex justify-content-between align-items-center list-item emailListItemadd" onClick={onReadEmail}>
                        ...
                        <h5 className="mb-1 mt10">
                            {email.users_union.map((element, keys) => (
                                element._id === myid && element.seen == 0 ?
                                    <span key={keys}><b style={{ fontSize: '14px' }}>{emailFrom.username === myusername ? 'Me' : emailFrom.username}</b></span> :
                                    element._id === myid && element.seen == 1 &&
                                    <span key={keys}>{emailFrom.username === myusername ? 'Me' : emailFrom.username}</span>
                            ))}
                        </h5>
        ...
        <div className="font-xs text-muted w-10 text-right width-20">
            ...
            <p className="pl10 email_datetime">
                {email.users_union.map((element, keys) => (
                    element._id === myid && element.seen == 0 ?
                        <span key={keys}><b style={{ fontSize: "13px" }}><span>{email.emails.create_Date}, {email.emails.create_Time}</span></b></span> :
                        element._id === myid && element.seen == 1 &&
                        <span key={keys} style={{ fontSize: "13px" }}>{email.emails.create_Date}, {email.emails.create_Time}</span>
                ))}
            </p>
            {/* {email.received_time} */}
        </div>
...
export default ArchiveEmailsListItem;
