...
// component
import EmailListItemRequestDoc from './EmailListItemRequestDoc';
...
class EmailListingRequestedDoc extends Component {
    ...
    getMails(myid) {
        axios.get(BaseUrl + 'api/composemail/requestedDocMails/' + myid)
            .then(res => {
    ...
    render() {
        const { currentAllSendedMails, username, myid } = this.state
            ...
            <div>
                ...
                <ul className="list-unstyled m-0">
                    {(this.state.currentAllSendedMails && this.state.currentAllSendedMails.length > 0 && this.state.currentAllSendedMails !== null) ? this.state.currentAllSendedMails.map((email, key) => (
                        <EmailListItemRequestDoc
                            email={email}
                            emailFrom={email.emails.froms}
                            emailTo={email.emails.to}
                            myusername={username}
                            myid={myid}
                            key={key}
